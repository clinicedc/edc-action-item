...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        qs = ActionItem.objects.filter(
            **opts).order_by("subject_identifier", "created")
        print(
            f"Found {qs.count()} action_items with missing `related_action_item`.")
        for action_item in ActionItem.objects.filter(**opts).order_by(
                "subject_identifier", "created"):
            print(action_item, action_item.action_cls.related_reference_fk_attr,
                  action_item.parent_action_item)
            model_cls = django_apps.get_model(
                action_item.action_cls.reference_model)
            try:
                ...
                if not self.dry_run:
                    ...
                    print("  not saving")
                print(
                    f"  - updated, set related_action_item={related_action_item}")
            if not self.dry_run:
                model_cls.save()
            else:
                print("  not saving")
